...
use cosmwasm_std::{attr, BlockInfo, Coin, ContractInfo, Env, HumanAddr, OwnedDeps, Uint128};

use crate::contract::{handle, init};
use crate::test_util::setup_wasm_utils_zk_circuit;
use protocol_cosmwasm::mixer::{DepositMsg, HandleMsg, InitMsg, WithdrawMsg};
...
fn test_mixer_should_work_with_wasm_utils() {
    ...
    .unwrap();
    assert_eq!(response.attributes.len(), 3);
    let on_chain_root = crate::state::read_root(&deps.storage, 1);
