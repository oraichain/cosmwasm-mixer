...
>(
	...
	let ck = CommitterKey::deserialize(ck_bytes.as_slice()).unwrap();
...
pub(crate) fn verify<E: PairingEngine, P: TEModelParameters<BaseField = E::Fr>>(
	pvk_bytes: Vec<u8>,
	vk_bytes: Vec<u8>,
	proof_bytes: Vec<u8>,
	public_bytes: Vec<u8>,
) -> Result<(), Error> {
	let vk = VerifierKey::deserialize(vk_bytes.as_slice()).unwrap();

	let proof = Proof::deserialize(proof_bytes.as_slice()).unwrap();

	let verifier_key = PlonkVerifierKey::deserialize(pvk_bytes.as_slice()).unwrap();
