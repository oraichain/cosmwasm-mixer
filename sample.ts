...
// }
const noteSecrets = [
  '0-HXoIWMmNaI2btxzOB2B6UY7LIrgN71XIIDNdlyGWU2wZOg9msElhfMBFMGytBemVY1KiXJgVm4JzxkSblgR9zQ==',
  '1-AvKMZekNDvDyoF3M8uIMiOHXPsvbTcHJx7sdsZdKS2nAMqiO3yAioFKWST1KWvuw5oPDcgIEigg5BKJOX9AYGw==',
  '2-Lzy9xtlvbIrJ9rS1Q3ZBCr2OdVIcxRCjo0eYc0ZS99zG6gg1lu/KJwpDi7MVU51LBZSf4flRWyQcXMSmj1BzyQ==',
  '3-kB8/BQlFwLWiBvt7G9iEsga/kXiSEpR3jEiihw4VaLOlmhZ98HqOH2uer2jU4M8sfve0GVLsPu8vzlwgkpkWTQ==',
  '4-s7m+OYB2LAJ10CPAE09thR9HH6DnJhUvvwpTB3T/3wW0FVAbRvUiQVCGbQgJc9iRQxq/upVNDFjXJSXA+IGhRA==',
  '5-236cb5MBCAfxLmL70aky4S+08zDex2FxP0xRQVbDKKNn2AQeaBd2T+jxirX+6sdnquhe6DY5rMVKtEc64bjEqQ==',
  '6-TiXmUC84O3I92RgKatFx3QYp6iYX07s78EYE0J5YaWgc1em91u6bVP6/giIuBArftvt/VQexT4yFpRXCvDJW7Q==',
  '7-xPzvh1grKoYumCb4n9xhqYwKhvmMoRkjxwZikbw4zUoQl71fMEfRbGr7qB4h9Fdu406GVALyM7xoccNYxyJspg==',
  '8-GkesyRfpUROHh7qPQfleEloQMcliziEKB5dyaE4uJPaMXiSkx1XlasOKUj/+GwmXwW2F3uOE7wlvtugxlbDrGQ==',
  '9-lLxMwSkeMx9wHpt0a85DH7n+hpm6ElJGetbH7DW+lDVnn1y83ohUosrngWJPSygjzXM5802H2S9Hz9NabQoHyg=='
];

const getLeaves = async (address: string): Promise<any[]> => {
  ...
  return txs.map((tx: any) => tx.body.messages[0].msg.deposit.commitment);
};
...
const getProof = async (index = 0): Promise<Uint8Array[]> => {
  const [leafIndex, secret] = noteSecrets[index].split('-');
  const noteSecret = Buffer.from(secret, 'base64'); // cosmwasmMixer.gen_note();
  const leaves = await getLeaves(address);
  return cosmwasmMixer.gen_zk(
    noteSecret,
    leaves.slice(0, parseInt(leafIndex)),
    address,
    sender
  );
};
